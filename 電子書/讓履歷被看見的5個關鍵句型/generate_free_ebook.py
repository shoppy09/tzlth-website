...
v5 — 句型一例3計算修正（2026-04-26）
  v3 既有修正（保留）：
    Fix A：移除「眼動研究顯示」（無來源），改為保守說法
    Fix B：B2B 範例「年營收」→「年化合約金額」（避免歧義）
    Fix C：PAR 例一 NT$30萬 → NT$14萬
    Fix D：PAR 例二 客戶留存率補基準（從 72% 提升至 90%）
    Fix E：結語 AI 診斷 CTA 視覺提升（淡藍底色框）
  v4 新增修正：
    Fix F：PAR † 估算基準邏輯修正：(6−0.5)×12=66人天 × NT$2,000 = NT$13萬
    Fix G：PAR 例二「客戶留存率從 72%→90%」→「客訴處理滿意度從 61%→84%」
  v5 新增修正：
    Fix H：句型一例3 計算修正（與完整版同步）：
            NT$24萬→NT$12萬，腳注 9天→5天，NT$216,000→NT$120,000
            正確計算：2週→5天，縮短5天×12人×NT$2,000=NT$120,000≈NT$12萬
"""
...
para(
    ...
    '選用句型一和句型四，是因為這兩個能處理覆蓋最廣的兩類困境：'
    '沒有數字的描述型句子（句型一），以及空窗期、短暫工作等需要主動說明的情況（句型四）。'
...
# Fix H：人事行政例3 計算修正 NT$12萬（2週縮短至5天，差5天）
add_footnote(
...
bullet('句型三：PAR 結構句型 — 展現主動性與解決問題能力，用問題→行動→結果說服 HR')
...
out_v5   = os.path.join(base, '讓履歷被看見的關鍵句型_免費精華版_v6.docx')
doc.save(out_main)
doc.save(out_v5)
