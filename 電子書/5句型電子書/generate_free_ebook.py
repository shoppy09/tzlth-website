...
v4 — 數字邏輯修正（2026-04-26）
  ...
  v4 新增修正：
    Fix F：PAR † 估算基準邏輯修正：(6−0.5)×12=66人天 × NT$2,000 = NT$13萬
            （扣除自動化後仍需 0.5 人天，避免「節省 100%」的不合理假設）
    Fix G：PAR 例二「客戶留存率從 72%→90%」→「客訴處理滿意度從 61%→84%」
            （留存率為年度/季度指標，3 個月內+18ppt 缺乏現實依據）
"""
...
out_v4   = os.path.join(base, '讓履歷被看見的關鍵句型_免費精華版_v4_數字邏輯修正.docx')
doc.save(out_main)
doc.save(out_v4)
print(f'✅ 主檔：{out_main}')
print(f'✅ v4 命名：{out_v4}')
