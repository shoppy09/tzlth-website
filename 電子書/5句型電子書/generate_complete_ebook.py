...
for cell, txt, fill in zip(cmp_hdr, ['', '免費精華版', '完整版（本冊）'],
    ...
    tcPr.append(shd)
cmp_rows = [
    ('適合你是', '有穩定工作經歷，主要困境是「做了很多但不知道怎麼寫」', '含跨界、空窗、頻繁換工作、ATS 卡關等複雜背景的求職者'),
    ('解決的問題', '如何把已有的工作內容，改寫成有說服力的成果句', '如何讓 HR 看懂你的背景，並通過多種篩選關卡'),
    ('句型數量', '2 個（句型一、句型三）', '5 個，完整涵蓋'),
...
bullet('管道二：詢問前同事或主管 — 離職前後，請主管或同事確認你記憶中的數字是否大致正確', size=10)
...
out_v5   = os.path.join(base, '讓履歷被看見的5個關鍵句型_完整版_v5_全面改版.docx')
doc.save(out_main)
doc.save(out_v5)
