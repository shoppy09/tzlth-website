...
v4 第三次評估修正後（2026-04-26）
  ...
  v3 新增修正（第三次評估報告）：
    Fix 9： 句型一例3 計算修正：NT$24萬→NT$12萬，腳注9天→5天/NT$216,000→NT$120,000
    Fix 10：句型二例1 補基準值：「通過率提升35%」→「從62%提升至84%（+22個百分點）」
    Fix 11：PAR例4 改前 新增產業脈絡「（客服中心）」
    Fix 12：PAR例4 改後 NPS→NPS（淨推薦分數）兩處
    Fix 13：句型四 新增延伸閱讀交叉引用
  v4 新增修正（第四次評估報告）：
    Fix 14：句型四 延伸閱讀框「本書附錄」→「搭配指南（獨立文件）」（附錄不存在）
    Fix 15：PAR例3 交期達成率起點 61%→58%（消除與例2客訴滿意度61%視覺重複）
"""
...
out_v4   = os.path.join(base, '讓履歷被看見的5個關鍵句型_完整版_v4_第三次評估修正後.docx')
doc.save(out_main)
doc.save(out_v4)
